...
```bash
localitas-core app-store add --name mmoney --compose ./docker-compose.yml --port 9220
localitas-core app-store start mmoney
```

Or open the App Store UI (package icon, top-right nav) and paste the `docker-compose.yml`.

The image is published to `ghcr.io/localitas/localitas-app-mmoney:latest`. To publish a new version:
